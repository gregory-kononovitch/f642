...
Brodger::Brodger(int width, int height, int numOsc)
{
    ...
    oscs = (Osc*) malloc(numOsc * sizeof(Osc));
    for (o = 0; o < numOsc; o++) {
        Osc osc = Osc(o % 3, 1L * width - 1L * width * rand() / RAND_MAX, 1L * height - 1L * height * rand() / RAND_MAX,
                50 //20L+ 1L*width * rand() / RAND_MAX
                , 1, 5, SIN //4L * rand()/RAND_MAX
                );
        __builtin_memcpy(&oscs[o], &osc, sizeof(Osc));
    }
}
...
//        oscs[o].decTyp  = NONE;
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            p.y = y;
            for (o = 0; o < numOsc; o++) {
                if (oscs[o].mas > 0) pix[oscs[o].col] += oscs[o].mas * oscs[o].coln1(&p);
            }
            for (o = 0; o < 3; o++) {
                pix[o] /= mtot[o];
                *(im++) = pix[o];
    ...
    if (tv2.tv_usec - tv1.tv_usec < 0) printf("CPPBrodger done in %ld.%ld\n", tv2.tv_sec - tv1.tv_sec - 1,
            (1000000 + tv2.tv_usec - tv1.tv_usec) / 1000);
    else printf("CPPBrodger done in %ld.%ld\n", tv2.tv_sec - tv1.tv_sec, (tv2.tv_usec - tv1.tv_usec) / 1000);
}
